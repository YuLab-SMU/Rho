...
| `plans/active-2026-08-05-system-credential-and-simple-llm-settings-spec.md` | active; CRED-UX1/2/3/4A and later recovery packages implemented; CRED-KEYCHAIN-R1 lazy access, complete affected tests/review, and local unsigned `0.4.1-dev.1` no-prompt startup/settings acceptance passed 2026-08-18 | shared native system-credential semantics, Model settings, bounded Provider discovery, three-layer Connection/Model/Capability routing, destructive settings workflows, lazy selected-Provider credential access, and deterministic verification | preserve zero-read metadata projection, process-local non-secret observation, exact selected-Provider/no-fallback/backend enforcement, and separate Keychain items; no combined vault, secret cache, schema, credential source, permission, network, release, or CRED-UX4B/C worker/media authority |
