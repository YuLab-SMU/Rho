...
| `release/active-0.4.0-dev.27-candidate-checklist.md` | active replacement-candidate contract; CONV-3-R1 source, complete affected automation, version synchronization, and independent R3 review pass; integration and every candidate/installed gate remain open | sole `0.4.0-dev.27` identity, future exact-candidate evidence, installed-acceptance ledger, and GO/NO-GO decision | additive session JSON only; schema 12 and package versions remain fixed; foreign/stale IDs fall back by current-project membership; `dev.26` evidence is non-composable |
...
| `plans/active-2026-08-09-agent-conversation-concurrency-spec.md` | active; Issue #5 authorized end-to-end; CONV-1 through CONV-3 and hosted `dev.26` candidate passed, but installed acceptance rejected `dev.26`; CONV-3-R1 source/automation/R3 review pass | durable project-scoped Agent Conversation identity, exact-thread context, bounded multi-turn admission, exact-turn cancellation/approval isolation, broker resource scheduling, Retry/Delete, project-transition ordering, and selected-session recovery | `dev.27` integration, candidate, installed acceptance, and Issue closure remain; additive project-session JSON does not move schema, file-edit, AFO-1, BH4, Workspace, environment, release, or credential authority |
...
| `plans/active-2026-08-10-run-current-line-advance-repair-spec.md` | active; Issue #15 ISSUE-15-EDITOR-1 implementation, focused/adjacent automated validation, 40 compatible frontend contracts, Monaco/basic browser review, and post-verification contract review complete 2026-08-10; two pre-existing Linux path-case test failures and installed Windows acceptance open | current-line-only next-line cursor transition and editor focus over the implemented WP2 execution path | WP2 retains execution/provenance and document cursor ownership; editor-shortcuts retains binding admission; UX-FIX4 retains explicit Console-tab focus; no backend, schema, policy, mutation, project, or release authority |
...
bounded recovery replacement identity. Its future exact-source
matrix, cross-platform artifacts, immutable Draft, and installed-acceptance
ledger must come from one fresh `dev.27` exact-commit candidate run. Run
`31337666426` belongs only to rejected `dev.26`; no earlier run, notarization
receipt, hash, or artifact is composable with `dev.27`.
