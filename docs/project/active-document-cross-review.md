...
| `plans/active-2026-08-20-p2-2-read-only-broker-grants-spec.md` | active under owner-approved local-first exception; P2-2A schema/persistence and P2-2B trusted UI/fresh handles locally complete 2026-08-20 with stable/MSRV/browser/failure/recovery evidence; `0.4.1-dev.3`; P2-2C `project.fs.read` active; P2-1 hosted gate still open | schema-v13 dedicated plugin permission requests/grants/events, trusted grant UI, fresh opaque handles, project.fs.read, workspace.r.inspect, and network.fetch | P2-2C permits only bounded contained reads; live handles remain memory-only and exact-identity bound; do not reuse Agent/environment approval lanes; no write/process/arbitrary-R/credential/storage/contribution authority; final acceptance still requires P2-1 hosted evidence |
