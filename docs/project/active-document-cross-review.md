...
| `design/active-2026-08-14-plugin-runtime-phase-2-workspace-third-party-design.md` | active; P2-0/pure contracts complete; owner authorized the complete Phase 2 end state 2026-08-20; P2-1 source/local macOS evidence complete with Windows/Linux hosted acceptance open; P2-2A-F source/local macOS complete at `0.4.1-dev.3`/`rho.bridge 0.1.15`; P2-3A-E source/local macOS complete at `0.4.1-dev.4`/`rho.agent 0.1.6`; P2-4A/B1/B2 local stops complete with durable first enable at `0.4.1-dev.5`; no later P2-4 slice active at this checkpoint; hosted/cross-platform gates remain open | project-local third-party workspace plugin discovery, manifest/digest, isolated host, broker-mediated permission grants, constrained handles, UI contributions, audit, disable/uninstall, and upgrade | Wasm-only executable profile with no WASI/imports; capabilities and permissions remain distinct; B2 reuses the trusted enable/permission command lane and adds no new guest capability; restart/disable/uninstall/upgrade remain inactive; local source/App evidence is not hosted/three-platform acceptance; marketplace/signing/publisher/update remain Phase 3 |
...
| `plans/active-2026-08-20-p2-4-plugin-lifecycle-recovery-upgrade-spec.md` | active under owner-approved local-first exception; P2-4A/B1/B2 passed local stops with durable first enable at `0.4.1-dev.5`; no later P2-4 package active pending explicit amendment/cross-review; prior hosted/cross-platform gates remain open | schema v14 durable state/transitions/events/tombstones, immutable package cache, enable/restart, disable/crash/retry, recoverable uninstall, upgrade CAS and rollback, installed Phase 2 audit | B2 owns only exact cache-backed first enable on the existing trusted command/permission lane; restart/disable/uninstall/update/rollback and new guest authority remain out of scope; local updater signature is rehearsal-only; final acceptance requires exact three-platform evidence |
