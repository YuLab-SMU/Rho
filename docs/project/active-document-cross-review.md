...
| `plans/active-2026-08-20-p2-3-controlled-contributions-spec.md` | active under owner-approved local-first exception; P2-3A Manifest V2/bounded schema/Source/Panel pure contracts locally complete with stable/MSRV 187-test matrices; P2-3B contribution proxy + transactional registry active; P2-1/P2-2 hosted/cross-platform gates remain open | Manifest V2, no-import Guest ABI V2 contribution loop, transactional Tool/Source/Skill/Command/Viewer/named-panel registrations, trusted rendering and Agent projection | P2-3A created no product route/Agent/UI/persistence/authority; no dynamic registration, raw DOM/HTML/Tauri, provider/process/write/arbitrary-R/credential authority; P2-4 retains durable lifecycle/restart/upgrade ownership |
