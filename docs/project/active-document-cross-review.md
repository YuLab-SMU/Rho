...
| `plans/proposed-2026-08-20-p2-2-read-only-broker-grants-spec.md` | proposed; complete-Phase-2 product intent authorized, but activation and product code blocked on P2-1 Windows/Linux hosted acceptance | schema-v13 dedicated plugin permission requests/grants/events, trusted grant UI, fresh opaque handles, project.fs.read, workspace.r.inspect, and network.fetch | do not reuse Agent/environment approval lanes; no write/process/arbitrary-R/credential/storage/contribution authority; activation requires P2-1 gate and filename/status transition |
