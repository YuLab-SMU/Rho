...
| `plans/active-2026-08-18-p1-3-workspace-snapshot-viewer-spec.md` | active; P1-3 authorized by the continuing whole-P1 objective; implementation pending | Workspace scope completion, fixed Workspace Snapshot tool and application Viewer plugins, typed Workspace broker operation, Agent/Tauri parity, trusted viewer host injection, and P1-3 evidence | may migrate only `workspace.snapshot`/`get_workspace_snapshot` and `viewer_read_file`; broker/Ark and `read_viewer_file` remain authoritative; no schema, external runtime, DOM/Tauri plugin authority, or P1-4 default switch |
