...
| `plans/active-2026-08-20-p2-4-plugin-lifecycle-recovery-upgrade-spec.md` | active under owner-approved local-first exception; P2-4A-E3 local packaged checkpoints complete through `0.4.1-dev.10`; D2/E2/E3 Browser visual review remains open after local-file URL policy rejection; no package active at E3 stop; hosted/cross-platform gates remain open | schema v14 durable state/transitions/events/tombstones, immutable package cache, enable/restart, disable/crash/retry, recoverable uninstall, upgrade CAS and rollback, installed Phase 2 audit | E3 exact cache/fresh grants/pointer reversal/fixed UI and accepted-cache restart are locally complete; no source rewrite/install/remote update/publisher trust; F/G inactive |
