...
| `plans/active-2026-08-20-p2-4-plugin-lifecycle-recovery-upgrade-spec.md` | active under owner-approved local-first exception; P2-4A and P2-4B1 passed local stops; P2-4B2 durable first enable active; B3 and later P2-4 packages inactive; prior hosted/cross-platform gates remain open | schema v14 durable state/transitions/events/tombstones, immutable package cache, enable/restart, disable/crash/retry, recoverable uninstall, upgrade CAS and rollback, installed Phase 2 audit | B2 owns only exact cache-backed first enable on the existing trusted command/permission lane; restart/disable/uninstall/update/rollback and new guest authority remain out of scope; no generic plugin storage/install/marketplace/signing/remote update; final acceptance requires exact three-platform evidence |
