...
| `plans/active-2026-08-17-three-platform-automatic-updater-dev43-spec.md` | active `AUTO3-DEV43`; owner authorized automatic Windows/macOS/Linux updates and automated publication; implementation pending | fresh dev.43 runtime auto-install policy, transactional Linux AppImage replacement, three-platform evidence/candidate/publication, and permanent development endpoint | supersedes the Linux exclusion only for dev.43 onward; preserves immutable dev.42 Draft, Free Trial disclosure, Apple trust, stable 404, and exact signed-byte gates |
| `release/active-0.4.0-dev.43-three-platform-updater-checklist.md` | active; no dev.43 release state exists | sole dev.43 source/candidate/publication/live evidence ledger | automated gates replace human observation; no prior candidate asset or acceptance composes |
