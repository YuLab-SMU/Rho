...
| `plans/active-2026-08-15-tauri-native-updater-spec.md` | active; `UPDATER-1A` protected integration and `UPDATER-1B` immutable `dev.40` Draft/audit are complete; `UPDATER-1C-T1` source integrated through PR #80 and candidate run `31986077326` constructed the dev.41 Draft; the Draft-URL normalization repair and repeated independent audit pass on a pending protected-main branch | separately signed Tauri native updater for Windows x64 and macOS arm64: explicit-user install/restart, compiled public key, final-byte updater signatures, native manifest projection, exact updater acceptance, and the bounded `dev.41` test-target bridge | preserve manual-only network admission, final-byte signing order, Linux exclusion, isolated secrets, immutable `dev.40`/`dev.41` Draft assets, and the subordinate `release/active-0.4.0-dev.41-native-updater-acceptance-target-checklist.md`; normalize only audit-record URLs and stop before target publication until the repair integrates and protected validation repeats |
| `release/active-0.4.0-dev.41-native-updater-acceptance-target-checklist.md` | active `UPDATER-1C-T1` transport contract; source integrated and signed eleven-asset Draft constructed at `9ec8117b`; bounded canonical tag-URL repair, regression suite, and local two-Draft/four-signature audit pass pending protected integration | one fresh higher signed test target, a marker that excludes it from ordinary release projection, and a maximum-45-minute temporary Tauri development manifest for dual-platform `dev.40 -> dev.41` evidence | consumes the updater runtime/public key and final-byte evidence; RELEASE-NOTES-1 owns the body, About/Update V1 owns normal Pages/V1 schema, and `dev.40` retains its exact GO/NO-GO decision; no Release/asset/version mutation, and no target publication before protected repair integration |
