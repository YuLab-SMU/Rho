...
| `design/active-2026-08-14-plugin-runtime-phase-2-workspace-third-party-design.md` | active; P2-0 implemented 2026-08-19; bounded pure-contract review-remediation across P2-0 through P2-4 implemented, 153 tests/clippy/scoped-rustfmt/workspace-build/diff-check and second review pass 2026-08-19; executable-host/product/platform acceptance remains open | project-local third-party workspace plugin discovery, manifest/digest, isolated host, broker-mediated permission grants, constrained handles, UI contributions, audit, and disable/uninstall/upgrade; current authorization fixes only pure validation/state contracts and regression coverage | capabilities and permissions stay distinct; manifest never self-authorizes; no project code or privileged operation executes in the remediation slice; real host selection and installed-platform gates remain open; marketplace/signing/publisher/update remain Phase 3 |
...
| `plans/active-2026-08-20-source-install-diagnostic-script-spec.md` | active authorized slice; S1 script/fixture/README/CI wiring in progress on 2026-08-20; arm64 bootstrap wiring (PR #79 follow-up) landed in the slice; build lanes and release gates untouched | `scripts/install-from-source.sh` diagnostics (platform detection, dependency report with distro package maps, `--json`), cargo build orchestration, and prefix install for unsupported Linux platforms; never official installers, update channels, or autotools conventions | never auto-install dependencies or escalate privileges; never sign or bundle official artifacts; never touch the updater channel; platform support follows `runtime/ark.json` (linux-x64 and linux-arm64 both wired via `bootstrap-ark-linux.sh`; BSD rejected, macOS via official DMG); official AppImage/deb/DMG lanes keep full authority |
