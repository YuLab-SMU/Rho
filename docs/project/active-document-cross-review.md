...
| `design/active-2026-08-14-plugin-runtime-phase-2-workspace-third-party-design.md` | active; P2-0/pure contracts complete; owner authorized the complete Phase 2 end state 2026-08-20; P2-1 source/local macOS evidence complete with Windows/Linux hosted acceptance open; P2-2A/B/C/D locally complete at `0.4.1-dev.3` and `rho.bridge 0.1.15`, P2-2E active; later packages sequentially gated | project-local third-party workspace plugin discovery, manifest/digest, isolated host, broker-mediated permission grants, constrained handles, UI contributions, audit, and disable/uninstall/upgrade | Wasm-only executable profile with no WASI/imports; capabilities and permissions remain distinct; P2-1 exposes only echo/diagnostic; P2-2 through P2-4 must pass separate gates; marketplace/signing/publisher/update remain Phase 3 |
| `plans/active-2026-08-20-p2-1-wasm-isolated-host-spec.md` | active; implementation, stable/MSRV/local matrix, independent security review, and local arm64 packaged-app smoke complete 2026-08-20; Windows/Linux hosted installed acceptance pending | exact Wasmtime host, Guest ABI V1, no-import boundary, fuel/memory/epoch limits, cancellation, trap/quarantine, and source/installed negative probes | no desktop routing, grant, privileged operation, contribution, storage, schema, or Phase 2 acceptance; P2-2 remains unactivated until hosted P2-1 gate closes |
| `plans/active-2026-08-20-p2-2-read-only-broker-grants-spec.md` | active under owner-approved local-first exception; P2-2A/B/C/D locally complete 2026-08-20 with stable/MSRV/browser/filesystem/Workspace/revoke/recovery evidence; `0.4.1-dev.3`, `rho.bridge 0.1.15`; P2-2E `network.fetch` active; P2-1 hosted gate still open | schema-v13 dedicated plugin permission requests/grants/events, trusted grant UI, fresh opaque handles, project.fs.read, workspace.r.inspect, and network.fetch | P2-2E permits only bounded credential-free public HTTPS GET/HEAD; live handles remain memory-only and exact-identity bound; do not reuse Agent/environment approval lanes; no write/process/arbitrary-R/credential/storage/contribution authority; final acceptance still requires P2-1 hosted evidence |
| `plans/proposed-2026-08-20-p2-3-controlled-contributions-spec.md` | proposed; complete-Phase-2 product intent authorized, activation blocked until P2-1/P2-2 acceptance | Manifest V2, no-import Guest ABI V2 contribution loop, transactional Tool/Source/Skill/Command/Viewer/named-panel registrations, trusted rendering and Agent projection | no dynamic registration, raw DOM/HTML/Tauri, provider/process/write/arbitrary-R/credential authority; P2-4 retains durable lifecycle/restart/upgrade ownership |
