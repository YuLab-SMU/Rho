...
| `plans/active-2026-08-15-tauri-native-updater-spec.md` | active; `UPDATER-1A` protected integration and `UPDATER-1B` immutable `dev.40` Draft/audit are complete; `UPDATER-1C-T1` source transport and deterministic validation are pending protected integration | separately signed Tauri native updater for Windows x64 and macOS arm64: explicit-user install/restart, compiled public key, final-byte updater signatures, native manifest projection, exact updater acceptance, and the bounded `dev.41` test-target bridge | preserve manual-only network admission, final-byte signing order, Linux exclusion, isolated secrets, immutable `dev.40` Draft, and the subordinate `release/active-0.4.0-dev.41-native-updater-acceptance-target-checklist.md`; stop before human installed acceptance, `dev.40` publication, or a permanent Pages native endpoint |
| `release/active-0.4.0-dev.41-native-updater-acceptance-target-checklist.md` | active `UPDATER-1C-T1` transport contract; source implementation/deterministic validation pending protected integration; target not constructed | one fresh higher signed test target, a marker that excludes it from ordinary release projection, and a maximum-45-minute temporary Tauri development manifest for dual-platform `dev.40 -> dev.41` evidence | consumes the updater runtime/public key and final-byte evidence; RELEASE-NOTES-1 owns the body, About/Update V1 owns normal Pages/V1 schema, and `dev.40` retains its exact GO/NO-GO decision; no ordinary publication or `dev.40` mutation |
