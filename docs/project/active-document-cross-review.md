...
| `plans/active-2026-08-17-stable-0.4.0-release-spec.md` | active `STABLE-040`; owner authorized stable construction/publication after every exact gate passes | stable SemVer identity, SemVer-derived GitHub release state, stable/development updater projection, and no-reviewer automated publication | rebuild all platform bytes; preserve dev.43, Free Trial trust disclosure, Apple trust, exact signed-byte gates, package versions, and runtime behavior |
| `release/active-0.4.0-stable-release-checklist.md` | active; owner GO authorization recorded, exact stable source/candidate/publication evidence pending | sole `0.4.0` stable identity, exact candidate, decision, public Release, and stable-channel evidence ledger | failed or partial candidate is NO-GO; no prior artifact/evidence composes; final facts update only after they are true |
