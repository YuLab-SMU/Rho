...
| `plans/active-2026-08-20-p2-2-read-only-broker-grants-spec.md` | active under owner-approved local-first exception; P2-2A/B/C locally complete 2026-08-20 with stable/MSRV/browser/containment/revoke/recovery evidence; `0.4.1-dev.3`; P2-2D `workspace.r.inspect` active; P2-1 hosted gate still open | schema-v13 dedicated plugin permission requests/grants/events, trusted grant UI, fresh opaque handles, project.fs.read, workspace.r.inspect, and network.fetch | P2-2D permits only fixed bounded Workspace inspection; live handles remain memory-only and exact-identity bound; do not reuse Agent/environment approval lanes; no write/process/arbitrary-R/credential/storage/contribution authority; final acceptance still requires P2-1 hosted evidence |
