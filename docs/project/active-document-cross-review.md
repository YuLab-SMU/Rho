...
| `plans/active-2026-08-20-p2-4-plugin-lifecycle-recovery-upgrade-spec.md` | active under owner-approved local-first exception; P2-4A-F local packaged checkpoints complete through `0.4.1-dev.11`; Browser visual review remains open after local-file URL policy rejection; G final hosted/installed audit active | schema v14 durable state/transitions/events/tombstones, immutable package cache, enable/restart, disable/crash/retry, recoverable uninstall, upgrade CAS and rollback, installed Phase 2 audit | G exact-head branch/Draft PR, Draft Fast and read-only six-leg dispatch only; no product authority, merge/release/publication; acceptance remains evidence-gated |
