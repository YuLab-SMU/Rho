...
| `design/active-2026-08-14-plugin-runtime-phase-2-workspace-third-party-design.md` | active; P2-0/pure contracts complete; owner authorized the complete Phase 2 end state 2026-08-20; P2-1 source/local macOS evidence complete with Windows/Linux hosted acceptance open; P2-2A-F source/local macOS complete at `0.4.1-dev.3`/`rho.bridge 0.1.15`; P2-3A-E source/local macOS complete at `0.4.1-dev.4`/`rho.agent 0.1.6`; P2-4A schema-v14 persistence and P2-4B1 exact snapshot/cache foundation local stops complete; no later P2-4 slice active at this checkpoint; hosted/cross-platform gates remain open | project-local third-party workspace plugin discovery, manifest/digest, isolated host, broker-mediated permission grants, constrained handles, UI contributions, audit, disable/uninstall, and upgrade | Wasm-only executable profile with no WASI/imports; capabilities and permissions remain distinct; B1 is broker-only cache evidence with no enable/restart/UI/guest authority; local source/App evidence is not hosted/three-platform acceptance; P2-4 remains sequential; marketplace/signing/publisher/update remain Phase 3 |
| `plans/active-2026-08-20-p2-1-wasm-isolated-host-spec.md` | active; implementation, stable/MSRV/local matrix, independent security review, and local arm64 packaged-app smoke complete 2026-08-20; Windows/Linux hosted installed acceptance pending | exact Wasmtime host, Guest ABI V1, no-import boundary, fuel/memory/epoch limits, cancellation, trap/quarantine, and source/installed negative probes | no desktop routing, grant, privileged operation, contribution, storage, schema, or Phase 2 acceptance; owner-approved local-first later work does not close or waive the P2-1 hosted gate |
| `plans/active-2026-08-20-p2-2-read-only-broker-grants-spec.md` | active under owner-approved local-first exception; P2-2A-F source/local macOS complete 2026-08-20 with stable/MSRV/browser/filesystem/Workspace/SSRF/revoke/recovery and current-source arm64 App/DMG smoke; `0.4.1-dev.3`, `rho.bridge 0.1.15`; hosted six-leg and Windows/Linux/three-platform installed acceptance open | schema-v13 dedicated plugin permission requests/grants/events, trusted grant UI, fresh opaque handles, project.fs.read, workspace.r.inspect, and network.fetch | no new P2-2 authority after local closeout; live handles remain memory-only and exact-identity bound; no write/process/arbitrary-R/credential/storage/contribution authority; source/local evidence does not authorize final Phase 2 acceptance or distribution |
| `plans/active-2026-08-20-p2-3-controlled-contributions-spec.md` | active under owner-approved local-first exception; P2-3A-E source/local macOS implementation complete at `0.4.1-dev.4`/`rho.agent 0.1.6` with stable/MSRV/full Rust+R, Browser, candidate/legacy App/DMG smoke; hosted six-leg, Windows/Linux/three-platform and independent final review open | Manifest V2, no-import Guest ABI V2 contribution loop, transactional Tool/Source/Skill/Command/Viewer/named-panel registrations, trusted rendering and Agent projection | no dynamic registration, raw DOM/HTML/Tauri, provider/process/write/arbitrary-R/credential authority; local closeout does not authorize final Phase 2 acceptance/distribution; P2-4 retains durable lifecycle/restart/upgrade ownership |
| `plans/active-2026-08-20-p2-4-plugin-lifecycle-recovery-upgrade-spec.md` | active under owner-approved local-first exception; P2-4A schema-v14 persistence and P2-4B1 exact snapshot/cache foundation passed local stops; no later B/P2-4 package active pending explicit amendment/cross-review; prior hosted/cross-platform gates remain open | schema v14 durable state/transitions/events/tombstones, immutable package cache, enable/restart, disable/crash/retry, recoverable uninstall, upgrade CAS and rollback, installed Phase 2 audit | B1 is read-only discovery plus broker-owned app-data copy, with no host/grant/contribution/UI or guest-visible cache access; no generic plugin storage/install/marketplace/signing/remote update; dedicated destructive lane and BH2/BH4 ownership preserved; final acceptance requires exact three-platform evidence |
| `design/active-2026-08-19-plugin-runtime-phase-2-5-agent-authored-evolution-design.md` | active design; bounded P2.5-0-through-P2.5-6 pure-contract review-remediation implemented with shared 153-test/clippy/scoped-rustfmt/workspace-build/diff evidence and second review pass 2026-08-19; executable Phase 2.5 remains unauthorized pending accepted Phase 2 prerequisites | Agent-authored and self-evolving project plugins: experience trace, recipe/Skill promotion, candidate builder, protected evaluation, lineage, standing policy, repair/rollback, capability gardener; current slice owns only pure predicates and negative tests | package digest remains executable identity; builder cannot self-authorize; no real observation, schema, Agent call, build, execution, production grant, activation, autonomous evolution, or UI; first-party promotion remains ordinary repository/release governance |
