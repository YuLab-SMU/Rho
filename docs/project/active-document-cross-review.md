...
| `plans/active-2026-08-20-p2-4-plugin-lifecycle-recovery-upgrade-spec.md` | active under owner-approved local-first exception; P2-4A-F local packaged checkpoints complete through `0.4.1-dev.11`; Browser visual review remains open after local-file URL policy rejection; no package active at F stop; hosted/cross-platform gates remain open | schema v14 durable state/transitions/events/tombstones, immutable package cache, enable/restart, disable/crash/retry, recoverable uninstall, upgrade CAS and rollback, installed Phase 2 audit | F exact recovery ordering/project revision/recovery-required and bounded transition inspection are locally complete; no new authority/install; G inactive |
