...
| `release/active-0.4.0-dev.24-candidate-checklist.md` | active published candidate record; exact candidate, signed/notarized DMG, installed acceptance, MAC5 GO, protected Release run `31297462728`, and live update run `31297482853` pass | sole `0.4.0-dev.24` immutable asset/evidence binding, installed-acceptance ledger, publication evidence, and GO/RELEASED decision | preserve release/tag/update evidence until a later single-use candidate supersedes it |
...
| `plans/active-2026-08-08-task-rail-mode-status-semantics-spec.md` | active; Issue #9 TASK-RAIL-SEMANTICS-1 implementation, complete affected frontend validation, exact browser/keyboard review, contract review, and dev.24 reconciliation complete 2026-08-08; installed acceptance open | Task Rail-only separation of mode shape/accessibility, status color/name, and risk ownership over existing turn data | source mandatory stop reached; UX4-P2 retains selection/task authority, Agent/broker retain mode/status/approval/execution truth, no schema or durable-state change, and exact installed-app acceptance remains separate |
| `plans/active-2026-08-09-agent-conversation-concurrency-spec.md` | active; Issue #5 authorized end-to-end 2026-08-09; CONV-1 and CONV-2 source checkpoints accepted 2026-08-09; stopped before CONV-3 activation | durable project-scoped Agent Conversation identity, exact-thread context, bounded multi-turn admission, per-turn cancellation/approval isolation, and broker resource scheduling | CONV-2 owns the accepted two-turn read-only admission, exact-turn cancellation/waiter ownership, serialized Workspace reads, and matching aggregate UI/mock state; Act mutation scheduling, Retry, Delete, release, and installed acceptance remain outside the accepted package and CONV-3 is inactive |
...
before Issue #9 advanced the live combined identity to `0.4.0-dev.24`. MAC4 implemented parallel candidate
...
authoritative main-repository candidate or draft exists, and MAC4 may not
perform MAC5 installed acceptance or publish a draft.
...
The active `0.4.0-dev.24` checklist carries those behaviors forward and adds
only the Issue #9 Task Rail projection. It inherits unchanged automation gates,
not predecessor results, and therefore requires its own exact-source candidate
matrix, artifact, and installed-acceptance ledger; no earlier run,
...
release, or sequencing conflict. The active candidate still requires the
complete exact-candidate matrix and one clean-source local artifact before
installed/live-Provider acceptance; MAC5/publication remain unauthorized.
