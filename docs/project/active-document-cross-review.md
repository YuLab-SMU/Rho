...
| `plans/active-2026-08-20-p2-2-read-only-broker-grants-spec.md` | active under owner-approved local-first exception; P2-2A/B/C/D/E locally complete 2026-08-20 with stable/MSRV/browser/filesystem/Workspace/SSRF/revoke/recovery evidence; `0.4.1-dev.3`, `rho.bridge 0.1.15`; P2-2F combined acceptance active; P2-1 hosted gate still open | schema-v13 dedicated plugin permission requests/grants/events, trusted grant UI, fresh opaque handles, project.fs.read, workspace.r.inspect, and network.fetch | P2-2F adds no authority; it owns combined closeout. Live handles remain memory-only and exact-identity bound; no write/process/arbitrary-R/credential/storage/contribution authority; final acceptance still requires P2-1 hosted evidence |
