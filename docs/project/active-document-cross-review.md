...
| `plans/active-2026-08-18-p1-2-run-history-source-spec.md` | active; local implementation review and affected validation passed 2026-08-18; exact-head Rust Fast pending | fixed Run History plugin/source descriptor, host runs service binding, generic 1 MiB source registry, Store-backed broker façade, legacy/candidate command parity/fallback, project/generation isolation, and P1-2 evidence | `list_runs` alone is migrated through the candidate source; Store remains authoritative; only candidate absence/activation failure falls back; no Workspace tool, Viewer, schema, public protocol, or P1-3 behavior yet |
...
P1-1 completed its Draft stop gate. P1-2 is the active extension package and
its local implementation/review gate has passed; exact-head Rust Fast remains.
...
shape, and truthful non-availability errors remain authoritative. P1-3 is the
next separately activated package under the continuing whole-P1 objective;
P1-4 remains inactive.
