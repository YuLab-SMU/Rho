...
| `plans/active-2026-08-20-p2-4-plugin-lifecycle-recovery-upgrade-spec.md` | active under owner-approved local-first exception; P2-4A schema-v14 state/transition/event/tombstone persistence passed its local stop gate; no later P2-4 package active pending explicit amendment/cross-review; prior hosted/cross-platform gates remain open | schema v14 durable state/transitions/events/tombstones, immutable package cache, enable/restart, disable/crash/retry, recoverable uninstall, upgrade CAS and rollback, installed Phase 2 audit | P2-4A adds no live lifecycle/UI/cache/authority; no generic plugin storage/install/marketplace/signing/remote update; dedicated destructive lane and BH2/BH4 ownership preserved; final acceptance requires exact three-platform evidence |
