...
| `plans/active-2026-08-17-signpath-free-trial-two-stage-dev42-spec.md` | active `SP-FT2-DEV42`; owner authorized extended Free Trial use; strict binary configuration/secret, source implementation, and local affected validation complete; hosted validation/integration pending | dev.42-only two-stage signing topology, build modes, binary/installer requests, installed-byte evidence, and bounded progression to a permanent native development endpoint | consumes Issue #26 signing authority and UPDATER-1 runtime/public-key/acceptance lanes; preserves dev.40 Draft/dev.41 test target, Free Trial untrusted disclosure, macOS authority, stable 404, and no production-Foundation claim |
| `release/active-0.4.0-dev.42-two-stage-signing-checklist.md` | active source/candidate checklist; source identity synchronized and local matrix passes; no dev.42 artifact or Release exists | sole exact dev.42 source, two-stage candidate, installed/update acceptance, decision, publication, and permanent endpoint evidence | no prior candidate/hash/request/acceptance may be reused; every candidate/publication phase stops separately and requires exact evidence |
