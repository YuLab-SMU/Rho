...
| `design/active-2026-08-14-plugin-runtime-phase-2-workspace-third-party-design.md` | active; P2-0/pure contracts complete; owner authorized the complete Phase 2 end state 2026-08-20; P2-1 source/local macOS evidence complete with Windows/Linux hosted acceptance open; P2-2A-F source/local macOS complete at `0.4.1-dev.3`/`rho.bridge 0.1.15`; P2-3A-E source/local macOS complete at `0.4.1-dev.4`/`rho.agent 0.1.6`; P2-4A/B and complete C local stops through crash/Retry complete at `0.4.1-dev.7`; no later P2-4 slice active; hosted/cross-platform gates remain open | project-local third-party workspace plugin discovery, manifest/digest, isolated host, broker-mediated permission grants, constrained handles, UI contributions, audit, disable/uninstall, and upgrade | C3 removes route/handles before durable crash, blocks third crash in ten minutes, and exposes Retry only for exact crashed state; uninstall/update/rollback remain inactive; marketplace/signing/publisher/update remain Phase 3 |
...
| `plans/active-2026-08-20-p2-4-plugin-lifecycle-recovery-upgrade-spec.md` | active under owner-approved local-first exception; P2-4A/B/C complete locally through crash/Retry at `0.4.1-dev.7`; no later package active; hosted/cross-platform gates remain open | schema v14 durable state/transitions/events/tombstones, immutable package cache, enable/restart, disable/crash/retry, recoverable uninstall, upgrade CAS and rollback, installed Phase 2 audit | C3 owns stable crash classification/count/block and exact Retry only; uninstall/update/rollback remain inactive and no plugin/Agent can invoke Retry |
