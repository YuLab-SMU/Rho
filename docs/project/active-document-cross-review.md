...
| `plans/active-2026-08-15-tauri-native-updater-spec.md` | active; `UPDATER-1A`/`UPDATER-1B` complete; dev.41 candidate and target publication passed through run `31989055536`; rejection run `31989333325` stopped pre-Pages on terminal-LF parsing, and real-byte replay exposed the stale Windows signature filename | separately signed Tauri native updater for Windows x64 and macOS arm64: explicit-user install/restart, compiled public key, final-byte updater signatures, native manifest projection, exact updater acceptance, and the bounded `dev.41` test-target bridge | preserve immutable dev.40 source/dev.41 public target bytes, manual-only network admission, final-byte signing order, Linux exclusion, isolated secrets, and exact fixture cleanup; bounded repair only signature framing, generated filename consumption, and regressions before redispatch |
| `release/active-0.4.0-dev.41-native-updater-acceptance-target-checklist.md` | active `UPDATER-1C-T1`; exact twelve-asset dev.41 target is public/excluded from normal Update Site; one pre-deploy run failed, and both terminal-LF plus `windows-x86_64.sig` repairs pass real-byte regression locally | one fresh higher signed test target, a marker that excludes it from ordinary release projection, and a maximum-45-minute temporary Tauri development manifest for dual-platform `dev.40 -> dev.41` evidence | accept/preserve at most one terminal LF, mutate only payload, consume exact generated platform filenames, and require both real verifiers to reject for the public-key reason; human/final release gates intact |
