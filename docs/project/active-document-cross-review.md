...
| `design/active-2026-08-14-plugin-runtime-phase-2-workspace-third-party-design.md` | active; P2-0/pure contracts complete; owner authorized the complete Phase 2 end state 2026-08-20; P2-1 source/local macOS evidence complete with Windows/Linux hosted acceptance open; P2-2A-F source/local macOS complete at `0.4.1-dev.3`/`rho.bridge 0.1.15`; P2-3A-E source/local macOS complete at `0.4.1-dev.4`/`rho.agent 0.1.6`; P2-4A/B and C1 local stops complete at `0.4.1-dev.6`; P2-4C2 project/restart/shutdown teardown reuse active; hosted/cross-platform gates remain open | project-local third-party workspace plugin discovery, manifest/digest, isolated host, broker-mediated permission grants, constrained handles, UI contributions, audit, disable/uninstall, and upgrade | C2 preserves durable enabled intent while making old runtime authority stopped/non-routable and cannot block BH2; Retry/uninstall/update/rollback remain inactive; marketplace/signing/publisher/update remain Phase 3 |
...
| `plans/active-2026-08-20-p2-4-plugin-lifecycle-recovery-upgrade-spec.md` | active under owner-approved local-first exception; P2-4A/B/C1 complete locally at `0.4.1-dev.6`; C2 boundary teardown reuse active; C3 and later packages inactive; hosted/cross-platform gates remain open | schema v14 durable state/transitions/events/tombstones, immutable package cache, enable/restart, disable/crash/retry, recoverable uninstall, upgrade CAS and rollback, installed Phase 2 audit | C2 adds no command and preserves desired intent across stopped runtime teardown; BH2 must continue after forced invalidation; Retry/uninstall/update/rollback remain inactive |
