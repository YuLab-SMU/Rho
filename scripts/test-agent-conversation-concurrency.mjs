...
assert.match(spec, /CONV-1 and CONV-2 source\s+checkpoints accepted 2026-08-09/);
assert.match(spec, /must reject\s+concurrent Act Turns/);

assert.match(rust, /const MAX_CONCURRENT_AGENT_TURNS: usize = 2/);
assert.match(rust, /struct AgentTaskEntry \{[\s\S]*conversation_id: String,[\s\S]*mode: String,[\s\S]*handle:/);
assert.match(rust, /AGENT_CONVERSATION_BUSY/);
assert.match(rust, /AGENT_ACT_EXCLUSIVE/);
...
assert.match(rust, /AgentTaskEntry \{[\s\S]*conversation_id: conversation_id\.clone\(\),[\s\S]*mode: mode\.clone\(\)/);
assert.match(rust, /agent_admission_allows_two_read_only_conversations_and_rejects_a_third/);
assert.match(rust, /agent_admission_rejects_same_conversation_and_parallel_act/);
...
assert.match(js, /mode === "act" && active\.length > 0/);
